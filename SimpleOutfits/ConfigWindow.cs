...
using SimpleOutfits.Sheets;

namespace SimpleOutfits;

public class ConfigWindow(IClientState clientState, ITextureProvider textureProvider, IDataManager dataManager, ItemManager itemManager, OutfitManager outfitManager, CommunicatorService communicatorService, ModManager modManager) : SimpleWindow($"{nameof(SimpleOutfits)}#{nameof(ConfigWindow)}", "Simple Outfits") {
    ...
    public override void Draw() {
        ...
        ImGui.EndChild();
        ImGui.SameLine();
        if (ImGui.BeginChild("outfitViewer", ImGui.GetContentRegionAvail(), true)) {
            DrawOutfitView(selectedOutfit);
        }
    ...
    public void DrawOutfitView(IOutfit? outfitInterface) {
        ...
        if (outfit is ActiveOutfit) {
            ...
            if (ImGui.BeginPopup("popupSaveOutfit")) {
                ...
                if (!string.IsNullOrEmpty(popupSavedOutfitError)) {
                    ImGui.TextColored(ImGuiColors.DalamudRed, popupSavedOutfitError);
                }
                ...
                if (configs.Count > 0) {
                    ...
                    if (ImGui.IsItemHovered()) {
                        using (ImRaii.Tooltip()) {
                            ...
                            if (configs.Count > 1 && ImGui.GetIO().KeyShift == false) {
                                ImGui.TextDisabled("Hold SHIFT to show mod settings.");
                            }
                        }
                    }
    ...
    private bool StainButton(Stain? stain, Vector2 size, bool tooltip = true, bool isSelected = false) {
        ...
        if (tooltip && ImGui.IsItemHovered()) {
            ImGui.SetTooltip(stain.Name.ToMacroString());
        }
