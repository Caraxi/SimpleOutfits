...
namespace SimpleOutfitsPlugin.Services;
...
public class ItemManager {
    ...
    public static ItemId NothingId(EquipSlot slot) {
        return uint.MaxValue - 128 - (uint)slot.ToSlot();
    }

    public static ItemId SmallclothesId(EquipSlot slot) {
        return uint.MaxValue - 256 - (uint)slot.ToSlot();
    }

    public static ItemId NothingId(FullEquipType type) {
        return uint.MaxValue - 384 - (uint)type;
    }

    public static EquipItem NothingItem(EquipSlot slot) {
        return new EquipItem(Nothing, NothingId(slot), 0, 0, 0, 0, slot.ToEquipType(), 0, 0, 0);
    }

    public static EquipItem NothingItem(FullEquipType type) {
        return new EquipItem(Nothing, NothingId(type), 0, 0, 0, 0, type, 0, 0, 0);
    }

    public static EquipItem SmallClothesItem(EquipSlot slot) {
        return new EquipItem(SmallClothesNpc, SmallclothesId(slot), 0, SmallClothesNpcModel, 0, 1, slot.ToEquipType(), 0, 0, 0);
    }
    ...
    public EquipItem Resolve(FullEquipType type, CustomItemId id) {
        return id.IsItem ? Resolve(type, id.Item) : EquipItem.FromId(id);
    }
    ...
    public BonusItem Resolve(BonusItemFlag slot, BonusItemId id) {
        return IsBonusItemValid(slot, id, out var item) ? item : new BonusItem($"Invalid ({id.Id})", 0, id, 0, 0, slot);
    }
    ...
    public bool IsStainValid(StainId stain) {
        return stain.Id == 0 || Stains.ContainsKey(stain);
    }
    ...
    public bool IsOffhandValid(in EquipItem main, ItemId offId, out EquipItem off) {
        return IsOffhandValid(main.Type.ValidOffhand(), offId, out off);
    }
